...
class JpgConst
{
	...
	Defaults = {};		// Default values, when a new block is created
	...
	constructor(obj)
	{
		...
		if(obj.defaults) this.Defaults = obj.defaults;
	}
};
...
class JPG extends JpgSection
{
	...
	*/
	constructor(image)
	{
		...
		this.#promise = new Promise((res,rej)=>{
			...
			let ip = setInterval(()=>{
				...
				else if(fails++ > 30) {rej(); clearInterval(ip);}
			}, 100);
		});
	}
	...
	{
		return this.#jpgHeader.removeMarkerBlock(marker);
	}
	...
	#open(image)
	{
	  	...
	  	if(isString)
		{
			if(image.startsWith("http") || image.startsWith("blob:"))
			{
				fetch(image).then((r)=>{
					...
					{
						byteArrays.push(b.charCodeAt(i));
					}
		...
		else if(isArray)
		{
			...
			if(this.view.getUint16(0) !== JPGCONST['SOI'].Value) {console.error("This is not a JPEG (SOI missing)!"); return null;}
			if(this.view.getUint16(this.view.byteLength - 2) !== JPGCONST['EOI'].Value) {console.error("This is not a JPEG (EOI missing)!"); return null;}
			let offset = 2;
...
scripts = [
	BASEURL+"jpg/jpgconst.js", 
	BASEURL+"jpg/jpgdata.js", 
	BASEURL+"jpg/jpgheader.js"
].forEach(s=>{
